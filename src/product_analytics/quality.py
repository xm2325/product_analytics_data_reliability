...
# `ingested_at` is generated by v0.26 sources but remains backward-compatible
# for legacy callers: if absent, certification treats the event as arriving at
# event_ts. New reference builds always contain the explicit field.
REQUIRED_COLUMNS = {
    "event_id",
    "user_id",
    "product",
...
def certify_events_with_rejects(
    ...
    Processing time is validated when supplied. Legacy inputs without
    ``ingested_at`` are treated as immediate arrivals at ``event_ts`` so the
    pre-v0.26 API remains usable while the current generator exercises the full
    event-time/processing-time contract.
    """
